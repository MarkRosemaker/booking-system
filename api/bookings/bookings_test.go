...
func TestRespond(t *testing.T) {
	...
	tables := []struct {
		...
		{"?name=Arnold&date=2010-02-01&id=fake_ID",
			"400 Bad Request: course with id 'fake_ID' does not exist"},
		{fmt.Sprintf("?name=Arnold&date=2010-02-01&id=%s", cPast.ID()),
			"400 Bad Request: the course is in the past"},
		{fmt.Sprintf("?name=Arnold&date=%s&id=%s", today.AddDays(-15), cPast.ID()),
			"400 Bad Request: the course is in the past"},
		{fmt.Sprintf("?name=Arnold&date=%s&id=%s", cTest.Start(), cTest.ID()),
			successOn(cTest.Start())},
		{fmt.Sprintf("?name=Arnold&date=%s&id=%s", cTest.Start(), cTest.ID()),
			"400 Bad Request: you are already attending this class"},
		{fmt.Sprintf("?name=Arnold&date=%s&id=%s", today.AddDays(2), cTest.ID()),
